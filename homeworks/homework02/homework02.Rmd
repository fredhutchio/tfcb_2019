...
2. Show axis tick labels as 10^0^, 10^1^, 10^2^,10^3^, 10^4^, 10^5^ for each axis.
...
In problems 4--6, after each operation, assign the result back to the `data`
variable.

**10 points**

Select the following columns from the `data` variable you created above. 
`ensembl_gene_id`, 
...
Filter the result from Problem 4 to include only rows where each of the 12 numerical columns 
you selected above has 50 counts or more. This is a simple way to avoid genes that have
very low counts. You might be tempted to do this step separately for
each of the 12 columns. But you can write lot less code if you use the `filter_if`
...
After filtering in Problem 5, divide each of the above 12 numerical columns by the 
corresponding median value in each column. This median normalization is typically done in 
high-throughput experiments to normalize for sample-to-sample difference in read depth.
